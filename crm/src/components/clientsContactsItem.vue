<template>
    ...
    <li v-if="contact.type == 'Vk'" class="table__item item tooltip-body">
    ...
    <li v-else-if="contact.type == 'facebook'" class="table__item item tooltip-body">
        ...
        <div class="tooltip-popup">
            Facebook: <a href="{{contact.value}}" class="tooltip-popup-link">{{ contact.value }}</a>
        </div>
    </li>
    <!-- телефон -->
    <li v-else-if="contact.type == 'Телефон'" class="table__item item tooltip-body">
        <a href="{{contact.value}}" class="item__social-link">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g opacity="0.7">
        ...
        <div class="tooltip-popup">
            <a href="tel:{{contact.value}}" class="tooltip-popup-link">{{ contact.value }}</a>
        </div>
    </li>
    <!-- Почта -->
    <li v-else-if="contact.type == 'Email'" class="table__item item tooltip-body">
        <a href="{{contact.value}}" class="item__social-link">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="tooltip-popup">
            почта: <a href="mailto:{{contact.value}}" class="tooltip-popup-link">{{ contact.value }}</a>
        </div>
    </li>

    <!-- подсписок дополнительных контактов -->
    <li v-else class="table__item item tooltip-body">
        <a href="{{contact.value}}" class="item__social-link">
